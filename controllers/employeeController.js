...
exports.addEmployee = async (req, res) => {
    try {
        ...
        if (name && (name.length < 3 || name.length > 64)) {
            return res.status(400).json({ message: "Name must be between 3 and 64 characters." });
        }

        if (fullAddress && fullAddress.length < 6) {
            return res.status(400).json({ message: "Address must be at least 6 characters long." });
        }
        if (
            selfIntroduction &&
            (selfIntroduction.split(/\s+/).length < 30 ||
                selfIntroduction.split(/\s+/).length > 40 ||
                selfIntroduction.length > 540)
        ) {
            return res.status(400).json({
        ...
        const profileImage = req.files?.profileImage?.[0]?.filename
            ? `/uploads/employees/${req.files.profileImage[0].filename}`
            : null;
        const businessCard = req.files?.businessCard?.[0]?.filename
            ? `/uploads/employees/${req.files.businessCard[0].filename}`
            : null;
        const certificate = req.files?.Certificate?.[0]?.filename
            ? `/uploads/employees/${req.files.Certificate[0].filename}`
            : null;
...
exports.updateEmployeeById = async (req, res) => {
    try {
        ...
        if (
            ...
            (updates.selfIntroduction.split(/\s+/).length < 30 ||
                updates.selfIntroduction.split(/\s+/).length > 40 ||
                updates.selfIntroduction.length > 540)
        ) {
            return res.status(400).json({
                message: "Self-introduction must be between 30 and 40 words, and no more than 540 characters long.",
            });
        ...
        if (empID && empID !== existingEmployee.empID && !/^TPEID\d{6}$/.test(empID)) {
            return res.status(400).json({
                message: "Employee ID must start with 'TPE1D' followed by 6 digits (e.g., TPE1D123456).",
            });
        }
        ...
        const image = req.files?.image?.[0]?.filename
            ? `/uploads/employees/${req.files.image[0].filename}`
        ...
        const businessCard = req.files?.businessCard?.[0]?.filename
            ? `/uploads/employees/${req.files.businessCard[0].filename}`
            : existingEmployee.businessCard;
        const certificate = req.files?.Certificate?.[0]?.filename
            ? `/uploads/employees/${req.files.Certificate[0].filename}`
            : existingEmployee.Certificate;
        const processedFeedback = Array.isArray(feedback) ? feedback.join("\n") : feedback;
